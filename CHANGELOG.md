...
- **Resume recovery-block prompt injection.** `lib/resume-engine.js`'s `generateHandoffPrompt` read `<projectRoot>/.claude/coalhearth/session_handoff.json` — a file a cloned/untrusted repository fully controls, with no provenance check — and interpolated every field (`activePlan.goal`/`nextSteps`/`constraints`, `checklist[].task`, `modifiedFiles`, `inFlightAgents[]` fields, `sessionId`, `timestamp`, `transcriptPath`) RAW into a markdown directive block on the SessionStart channel. A `goal` containing a newline could forge CoalHearth's own trusted `> [!IMPORTANT]` callout syntax and carry an arbitrary instruction, byte-indistinguishable from the tool's authoritative voice (falsifier-proven: a planted journal made the shipped hook emit a `curl|sh` + exfiltration instruction verbatim). Fixed: every untrusted field now renders as plain text inside one fenced block (a new `fence()` helper sizes the fence past the longest backtick run in the content, so it can never be closed early), with an explicit trusted sentence stating the block is unverified snapshot data, never an instruction — nothing untrusted is interpolated outside the fence anywhere in the function. A findings-back round caught one field (`_orphanSweep.scratch`/`.worktrees`) still using a bare `|| 0` outside the fence, not reachable through either shipped hook today but a real latent gap; fixed with a strict `Number(...)` coercion. Honest bound, stated in the code: this closes the structural vector (no escape, no forged trusted syntax); whether a model acts on plainly-labeled reported text regardless is a property of the model, not something a hook can prove closed. — test: `scripts/lib/engine.test.mjs` (3 cases: forged-callout, backtick-run, non-numeric `_orphanSweep`), `bin/session-start.test.js` (1 end-to-end case through the real shipped hook)

### Fixed

- **Concurrent-lost-update lock race.** `lib/handoff-journal.js`'s `_acquireLock` treated any non-`EEXIST` error from the `wx`-flag lock-create as a permanent "unlockable" condition and bailed lock-free on the very first attempt, skipping its own wait/retry/steal loop entirely. On Windows a losing create race can surface as `EPERM`, not `EEXIST` — reproduced directly IN ISOLATION (not only under external load, contradicting this room's own `ROOT1/H1` test comment), ~12-18% of a 10-way concurrent fan-out silently dropped a journaled step. `EPERM` is now treated as the same "someone else has it" signal as a live `EEXIST` holder (poll and retry within the existing bounded `LOCK_WAIT_MS`), while a genuinely different error code still bails immediately (proven via an `EACCES` negative control). 60/60 and separately 50/50 clean sweeps after the fix, against a failure that was common before it. Both misleading comments the room's own 2026-08-22 HIGH had left standing (`handoff-journal.js`'s "500ms comfortably serializes... beyond ~20 writers" and the test's "never in isolation, not reproduced") are corrected to name the real mechanism. — test: `lib/handoff-journal.test.js` (2 cases: injected-EPERM-is-contention, injected-EACCES-still-bails), `bin/post-tool-use.test.js` `ROOT1/H1` (real 10-way concurrent spawn)
