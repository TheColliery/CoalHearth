...
- **The journal is a snapshot of YOUR session, kept on YOUR disk.** It records the goal, checklist, planned steps, and modified-file *names* (read best-effort from the `Write`/`Edit`/etc. tool-call payloads the hook observes — no `git` is run) — never uploaded anywhere. The budget figures are a local char-count estimate, advisory only, not a platform-verified read.
