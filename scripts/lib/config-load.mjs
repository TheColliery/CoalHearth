...
export function configNotices(opts) {
  try {
    ...
    const lines = [];
    // CWK-135 (a): the line names the path of the TIER that failed. The PROJECT tier keeps the verbatim
    // flock string (`canonical` = the own-dir canonical location); the GLOBAL tier names the global file's
    // OWN path (globalConfigPath -- CLAUDE_CONFIG_DIR-aware), because a global config has no project
    // location to move to and a project path there would send the user to the wrong place.
    const unreadable = (p, reason, canon) => 'UNREADABLE: ' + p + ' exists but is not a readable config (' + reason + '); it was skipped \u2014 canonical = ' + (canon || canonical);
