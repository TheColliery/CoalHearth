...
**Docs:** [thecolliery.gitbook.io/thecolliery-docs/tools/coalhearth](https://thecolliery.gitbook.io/thecolliery-docs/tools/coalhearth) *(publishing soon)*
...
1. `<project>/.<the running agent's OWN dir>/coal/coalhearth.json` — the dir of the agent actually executing (Claude Code = `.claude`, Antigravity = `.agents`, Gemini CLI = `.gemini`). **This is the canonical path — under Claude Code, exactly `.claude/coal/coalhearth.json`.**
2. Other known agent dirs, fixed order: `.claude` → `.agents` → `.gemini` (first **found** wins).
3. **DEPRECATED** — `<project>/.claude/.coalhearth.json` (the nested legacy shape). Still read, after every canonical path. Before this release the loader never read it at all.
4. **DEPRECATED** — `<project>/.coalhearth.json` at the project root (the pre-2026-08-08 shape). Still read, always last.

**Both legacy shapes are deprecated, and nothing breaks yet — but move your config.** Put the same content at `.claude/coal/coalhearth.json` (or the running agent's own dir), or set any value with `node scripts/configure.mjs` (below), which writes the canonical file and removes the legacy one for you. Deprecated at the 2.5.0 MINOR; removable no earlier than the next MAJOR. This room owns the migration. This README note and the CHANGELOG `### Deprecated` entry are the announcement — ship-text only, because a hook has no sanctioned channel of its own to announce on (Phoenix #13).

**What you see when a legacy path is actually hit.** That is a separate, smaller thing from the announcement: one line per finding on the SessionStart context-injection channel this room already holds sanctioned (the same one as the recovery block and the `language` directive), naming the path and the replacement — `[CoalHearth] LEGACY: <path> is deprecated but still read; canonical = .claude/coal/coalhearth.json`. A config-looking file at a path that is not in the read order above at all (say `.claude/coalhearth.json`, missing the `coal/`) gets `[CoalHearth] IGNORED: <path> is not a config path; canonical = .claude/coal/coalhearth.json`. A config that **is** at a path the walk reads but cannot be used gets `[CoalHearth] UNREADABLE: <path> exists but is not a readable config (<reason>); it was skipped — canonical = .claude/coal/coalhearth.json`, where `<reason>` is one of `malformed JSON`, `a directory`, `unreadable` (the OS refused the read) or `not a JSON object` (valid JSON that is an array, a string, a number or `null`). The global `~/.claude/.coalhearth.json` is checked too, and when it is the one that failed the line ends with the global file's own path instead (`canonical = <that path>`), because a global config has no project location to move to. An unreadable legacy file gets `UNREADABLE` only, never also `LEGACY`. "Skipped" means the file contributes no settings; it does not open the door to a lower-priority config, because the walk still stops at the first path that holds a file. The line is appended to whatever SessionStart is already saying, or printed on its own when the session would otherwise be silent. **It repeats every session for as long as the legacy file exists** — once per session, not once ever — so a nudge that keeps appearing is working, not stuck; migrating is what silences it. A readable canonical config on its own, or none at all, prints nothing.

**What it does not tell you: a config at a real path in the read order that simply loses is not announced.** Only the first config found is ever read, so a legacy file left beside a canonical one, a second agent dir's canonical file, or the root `.coalhearth.json` once the nested one exists is silently dead — an edit to it will not take. Migrate it or remove it. (A **directory** sitting at a path in the read order is different: it is passed over — the walk counts a path only if it is a file, and a directory named like a config no longer anchors the project root — and, unless it sits after the config that is read, it is reported as `UNREADABLE ... (a directory)` above; with no config read at all, every such directory is reported.)

**If you hold both legacy files, the nested one now wins.** Before this release only the root `.coalhearth.json` was read, so `.claude/.coalhearth.json` was dead; now it is read first. Your effective config can change without you editing anything — and the notice above names the file now being read, not the root file that just went dead, so check for both, keep the file you mean and migrate it.

Write target = wherever the config was found — except that a config found at **either** legacy path is migrated on write (`configure.mjs` writes the canonical file and removes the legacy one; a hook never moves a file on a mere read); nothing found anywhere = the running agent's own dir. **`coalhearth.json` still has no HOOK writer** — the hooks that run every session never write config, only read it; that half of "always hand-edited" stays true. What changed: [`scripts/configure.mjs`](scripts/configure.mjs) is now a USER-INVOKED CLI that edits it for you — `node scripts/configure.mjs --language th` — so "never written by CoalHearth itself" no longer describes the whole picture. Nothing runs it on your behalf; you or your agent runs it explicitly, the same way you'd run any other script in this repo.
