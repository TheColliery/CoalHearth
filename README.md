...
**What it does not tell you: a config at a real path in the read order that simply loses is not announced.** Only the first config found is ever read, so a legacy file left beside a canonical one, a second agent dir's canonical file, or the root `.coalhearth.json` once the nested one exists is silently dead — an edit to it will not take. Migrate it or remove it. (A **directory** sitting at a path in the read order is different: it is passed over — the walk counts a path only if it is a file, and a directory named like a config no longer anchors the project root — and, when it sits ahead of the config that is read, it is reported as `UNREADABLE ... (a directory)` above.)
