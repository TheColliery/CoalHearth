...
**What you see when a legacy path is actually hit.** That is a separate, smaller thing from the announcement: one line on the SessionStart context-injection channel this room already holds sanctioned (the same one as the recovery block and the `language` directive), naming the path and the replacement — `[CoalHearth] LEGACY: <path> is deprecated but still read; canonical = .claude/coal/coalhearth.json`. A config-looking file the loader does *not* read (say `.claude/coalhearth.json`, missing the `coal/`) gets `[CoalHearth] IGNORED: <path> is not a config path; canonical = .claude/coal/coalhearth.json`. The line is appended to whatever SessionStart is already saying, or printed on its own when the session would otherwise be silent. **It repeats every session for as long as the legacy file exists** — once per session, not once ever — so a nudge that keeps appearing is working, not stuck; migrating is what silences it. A clean canonical config, or none at all, prints nothing.

**If you hold both legacy files, the nested one now wins.** Before this release only the root `.coalhearth.json` was read, so `.claude/.coalhearth.json` was dead; now it is read first. Your effective config can change without you editing anything — keep the file you mean and migrate it.
