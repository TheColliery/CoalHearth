...
test('_acquireLock treats an injected EPERM on wx-create as contention, not a permanent failure', () => {
  const dir = tmpDir();
  const journal = new HandoffJournal({ outputDirectory: dir }, dir);
  const lockPath = path.join(dir, 'session_handoff.json.lock');
  const realWrite = fs.writeFileSync;
...
test('_acquireLock still bails lock-free immediately on a non-contention error (e.g. EACCES)', () => {
  const dir = tmpDir();
  const journal = new HandoffJournal({ outputDirectory: dir }, dir);
  const lockPath = path.join(dir, 'session_handoff.json.lock');
  const realWrite = fs.writeFileSync;
