...
test('project config found by walking up from a subdir', () => {
  const root = mk();
  const home = mk();
  fs.writeFileSync(path.join(root, '.coalhearth.json'), JSON.stringify({ budgets: { maxTurns: 7 } }));
  const sub = path.join(root, 'a', 'b');
  fs.mkdirSync(sub, { recursive: true });
  const cfg = loadConfig({ cwd: sub, home });
  assert.strictEqual(cfg.budgets.maxTurns, 7);
...
test('project overlays global, per-group shallow merge', () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ budgets: { maxTurns: 30, maxTokens: 2000000 } })
  );
  const proj = mk();
  fs.writeFileSync(path.join(proj, '.coalhearth.json'), JSON.stringify({ budgets: { maxTurns: 5 } }));
  const cfg = loadConfig({ cwd: proj, home });
  assert.strictEqual(cfg.budgets.maxTurns, 5, 'project overrides global');
  assert.strictEqual(cfg.budgets.maxTokens, 2000000, 'global key not overridden is kept');
  fs.rmSync(home, { recursive: true, force: true });
...
test('walk stops at home -- a .coalhearth.json ABOVE home is ignored', () => {
  const base = mk();
  fs.writeFileSync(path.join(base, '.coalhearth.json'), JSON.stringify({ budgets: { maxTurns: 999 } }));
