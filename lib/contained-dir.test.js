...
test('self-ignore: a normal custom outputDirectory leaf still gets self-ignored', () => {
  const root = mk();
  try {
    const dir = containedOutputDir('.claude/custom-ch', root);
    assert.ok(fs.existsSync(path.join(dir, '.gitignore')), 'a narrower custom leaf is still CH-owned and gets ignored');
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
